...
* Select the row and use Outline > Pin Row (<kbd>Command-Y</kbd>)

#### To unpin a row

* Select the pinned row and use Outline > Pin Row (<kbd>Command-Y</kbd>) again
...
Sometimes, though, you'd rather land on a row while still seeing it next to its siblings. Hold <kbd>Command</kbd> while you click and Bike focuses the item's *parent* instead, then selects the item inside it. The same trick works in the [choice box](using-the-choice-box.md): press <kbd>Command-Return</kbd> to focus a result's parent rather than the result itself.
